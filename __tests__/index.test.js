...
describe('BackgroundActions with multiple tasks', () => {
    ...
    test('defineTask and startTask register and manage tasks', async () => {
        ...
        expect(require('react-native').AppRegistry.registerHeadlessTask).toHaveBeenCalledWith(
            taskName,
            expect.any(Function)
        );
    });
    ...
    test('can run multiple tasks simultaneously', async () => {
        ...
        expect(BackgroundActions.getRunningTaskNames().sort()).toEqual(
            [task1Name, task2Name].sort()
        );
    });
